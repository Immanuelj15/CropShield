...
Expected data layout (standard torchvision ImageFolder format):
    ...
    data/plantvillage/
        ...
        Potato___Early_blight/
            ...
        ... (38 class folders total for the full PlantVillage set)

Get the data (run once, on your own machine):
    pip install kagglehub
    python -c "import kagglehub; print(kagglehub.dataset_download('abdallahalidev/plantvillage-dataset'))"
  then point --data_dir at the downloaded folder (or symlink it to data/plantvillage).

Usage:
    pip install torch torchvision scikit-learn matplotlib seaborn tqdm pillow
    python backend/services/train_disease_detection.py --data_dir data/plantvillage --backbone resnet18 --epochs 15
"""
...
def freeze_backbone(model, backbone: str, freeze: bool):
    """Freeze all layers except the final classifier head — useful for the
    first few epochs (fast, avoids destroying pretrained features), then
...
def main():
    ...
    parser.add_argument("--out_dir", default=str(OUT_DIR), help="output directory for checkpoints")
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs} [{mode}] "
              f"train_loss={train_loss:.4f} train_acc={train_acc:.4f} "
              f"val_loss={val_loss:.4f} val_f1={val_f1:.4f} ({elapsed:.1f}s)")

        history.append({"epoch": epoch + 1, "mode": mode, "train_loss": train_loss,
                         "train_acc": train_acc, "val_loss": val_loss, "val_f1": val_f1})
    ...
    with open(out_dir / "final_metrics.json", "w") as f:
        json.dump({"classification_report": report, "confusion_matrix": cm.tolist(),
                    "backbone": args.backbone, "best_val_f1": best_f1}, f, indent=2)
